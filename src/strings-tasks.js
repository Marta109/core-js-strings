...
function removeFirstOccurrences(str, value) {
  ...
  return str && substr > -1
    ? str.slice(0, substr) + str.slice(substr + value.length)
    : str;
}
...
function isPalindrome(str) {
  ...
  if (newStr === newStr.toLowerCase().split('').reverse().join(''))
    result = true;
  return result;
}
...
function encodeToRot13(str) {
  ...
  return str
    .split('')
    .map((el) => {
      if (el.match(/[.,~!?\s]/g)) return el;
      const index = codeStr.indexOf(el);
      return enCodeStr[index];
    })
    .join('');
}
